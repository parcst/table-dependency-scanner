...
```bash
PYTHONPATH=src python3 -m table_scanner --local-path /path/to/rails/app --table-name users
PYTHONPATH=src python3 -m table_scanner --repo org/repo --table-name orders --output results.csv
```

### CLI (installed as package)
```bash
pip install -e .
table-scan --local-path /path/to/rails/app --table-name users
```
...
5. **Reverse-association filtering** — drops `MODEL_HAS_MANY_REVERSE` and `MODEL_HAS_ONE_REVERSE` results. These arise when another model declares `has_many :table` or `has_one :singular`, meaning the **scanned table itself** holds a FK pointing to that other model's table (e.g. `orders.user_id → users`). This is the opposite direction from what the scanner answers ("which tables have a FK to `<table>.id`"), so including them produces misleading evidence entries.
...
The foreign key column is derived using proper Rails-style inflection from `inflection.py`. For example, `orders` → `order` (singular) → `order_id`. Handles common English patterns: `-ies`, `-ses`, `-xes`, `-ches`, irregular nouns (`people` → `person`, etc.).

### ModelScanner FK Direction

The `ModelScanner` correctly handles Rails association direction:
- `belongs_to :order` → FK lives on the **owner table** (the declaring model's table). Reports `table_name=owner, column_name=order_id`.
- `has_many :orders` / `has_one :order` → FK lives on the **target table** (`orders`). Reports `table_name=orders, column_name={owner_singular}_id` with reference type `MODEL_HAS_MANY_REVERSE` / `MODEL_HAS_ONE_REVERSE`.

This avoids false positives where `has_many :orders` in a parent model would incorrectly report a FK column on the parent table (a column that does not exist there).
...
`src/table_scanner/inflection.py` provides:
